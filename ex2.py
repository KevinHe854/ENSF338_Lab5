...
# has O(n long n) complexity, while in the second we are simply inserting the element where it belongs in order, which has O(n) complexity. Since 
# O(n) is more efficient than O(n log n), the second way is faster than the first.
